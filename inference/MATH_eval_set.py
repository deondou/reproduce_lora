...
def process_results(doc, completion, answer):
    ...
    if len(split_ans) > 1:
        ...
        if util.is_equiv(extract_ans, answer):
            ...
            'valid_id': len(valid_outputs) + 1,
            'question': doc,
            'response': completion,
            'model_answer': extract_ans,
...
def test_hendrycks_math(model, data_path, start=0, end=MAX_INT, batch_size=1, tensor_parallel_size=1,save_path='./'):
    hendrycks_math_ins = []
    ...
    with open(data_path, "r+", encoding="utf8") as f:
        for idx, item in enumerate(jsonlines.Reader(f)):
            ...
            golden_ans = item['golden_answer']
            # hendrycks_math_answers.append(temp_ans)
            hendrycks_math_answers.append(golden_ans)
    ...
    def save_results_to_jsonl(filename):
        with open(filename, 'w') as f:
            for entry in valid_outputs:
                f.write(json.dumps(entry) + '\n')
    save_results_to_jsonl(save_path+".jsonl")
...
def parse_args():
    ...
    parser.add_argument("--save_path", type=str, default='./')  # data path
    return parser.parse_args()

if __name__ == "__main__":
    args = parse_args()
    test_hendrycks_math(model=args.model, data_path=args.data_file, start=args.start, end=args.end, batch_size=args.batch_size, tensor_parallel_size=args.tensor_parallel_size, save_path=args.save_path)
